...
# ============================================================
# 函數 5：將圖片檔編碼為 Base64 字串 (原始 PNG，向後相容)
# ============================================================
def encode_image_to_base64(image_path: str) -> str:
    """
    讀取指定路徑的圖片，並回傳 Base64 編碼後的字串。
    此字串會被組合成 data URL (`data:image/png;base64,...`) 傳給 Vision Model。
    ⚠️ 注意：此函式不做任何壓縮，建議改用 encode_compressed_screenshot() 以節省 Token。
    """
...
# ============================================================
# 函數 5a：壓縮截圖後編碼為 Base64 (用於 Vision Model API)
# ============================================================
def encode_compressed_screenshot(image_path: str) -> str:
    """讀取截圖 → 無條件縮到目標寬度 → JPEG 壓縮 → 回傳 Base64 字串。

    為什麼不用原本的 PNG 編碼？
      - 1920x1080 PNG ≈ 3~5 MB，Base64 ≈ 4~7 MB → 數十萬 token
      - 同畫面 JPEG q60 ≈ 200~400 KB，Base64 ≈ 300~500 KB → 數萬 token
    ...
    Args:
        image_path: 截圖檔案路徑 (通常為 PNG)

    Returns:
        Base64 編碼的 JPEG 字串 (可直接用於 Vision Model API)
    """
    with Image.open(image_path) as img:
        ...
        # 2) 轉 RGB (PNG 可能有 RGBA/灰階，JPEG 不支援透明度)
        if img.mode != "RGB":
            img = img.convert("RGB")
...
def ask_vision_model(
    ...
    print(f"[編碼中] 將 {image_path} 壓縮為 JPEG 並轉為 Base64...")
    b64_image = encode_compressed_screenshot(image_path)
...
def ask_vision_model_grid(
    ...
    # (B) 編碼圖片
    # --------------------------------------------------------
    print(f"[編碼中] 將 {image_path} 壓縮為 JPEG 並轉為 Base64 (grid mode)...")
    b64_image = encode_compressed_screenshot(image_path)
