...
# 注意：截圖壓縮函式已移至 engine.encode_compressed_screenshot()
# 截圖壓縮設定常數已移至 config.py (SCREENSHOT_MAX_WIDTH, SCREENSHOT_JPEG_QUALITY)
# server.py 統一透過 engine 模組呼叫，避免邏輯重複
...
def get_screen_state(
    ...
    try:
        ...
        if include_screenshot and _state["last_screenshot_path"]:
            result["screenshot_base64"] = engine.encode_compressed_screenshot(
                _state["last_screenshot_path"]
            )
            logger.info(
                f"[Tool 1] 已附加截圖 ({len(result['screenshot_base64'])} chars, "
                f"JPEG q{wcmd_config.SCREENSHOT_JPEG_QUALITY})"
            )
